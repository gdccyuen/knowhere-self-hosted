...
- MinerU API Key，用于 PDF 文档的初始解析（如使用自托管 MinerU，则无需填写，详见[本地 MinerU 模式](#本地-mineru-模式)）。
...
如果你在本地或内网部署了 MinerU 实例，可以不依赖 `mineru.net`，在 `.env` 中加入：
...
该模式下 `MINERU_API_KEYS` 不是必填。容器通过 `host.docker.internal` 访问宿主机上的 MinerU；`compose.yaml` 已配置 `extra_hosts`，在 Linux、Docker Desktop 和 OrbStack 上均能解析。参数调优、分片并发和超时说明详见[配置参考 - 本地 MinerU 模式](docs/configuration.zh-CN.md#本地-mineru-模式)。
